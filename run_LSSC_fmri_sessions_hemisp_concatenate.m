...
if dataID == 1
    ...
    resultDir = 'D:\UCSD_Acads\ProfGal_Research\test_run_norm1_pca0_kNN16_sftune4_hem_concatenate_AllenOrdered';
end
if dataID == 2
    dataDir = 'D:\UCSD_Acads\ProfGal_Research\data_fMRI_GSR_processed\GSR_1voxelmask_new';
    resultDir = 'D:\UCSD_Acads\ProfGal_Research\data_fMRI_GSR_processed\GSR_1voxelmask_results\test_run_norm1_pca0_kNN16_sftune4_hem_concatenate_AllenOrdered';
end
if dataID == 3
    dataDir = 'D:\UCSD_Acads\ProfGal_Research\data_fMRI_GSR_processed\GSR_thickermask_new';
    resultDir = 'D:\UCSD_Acads\ProfGal_Research\data_fMRI_GSR_processed\GSR_thickermask_results\test_run_norm1_pca0_kNN16_sftune4_hem_concatenate_AllenOrdered';
end
if dataID == 4
    ...
    for ses = 1:length(sesList) % processing each session
        ...
        for i = 1:length(fileList) % processing each .nii file
            ...
            if ((i == 1 || strcmp(prev_sub, sub_value)) && i ~= length(fileList))    % runs belong to same subject
                ...
                V_combined_h = zeros(size(V_top, 1)+ROW_PAD, size(V_top, 2), 2*num_time_frames); % twice frames if you are concatenating hemispheres

                % Interleave the top and bottom hemispheres in time
                ...
                cfg.title_str = ['sub_' prev_sub '_session_' num2str(prev_ses)];
                cfg.outputfilepath = resultDir;
                %@renu
                cfg.pca = 0;
                cfg.normalize = 1;
                cfg.ComputeTemporalCorr = true;
                cfg.ReorderLikeAllen = true;
               
                % Run segmentation     
                hemisp = 1;
                [labels_all, mergedA_all] = runROI_meso_nlm_new_v2(cfg, ...
                    dFoF_masked, allregionspix, brain_mask, AllenFilePath, hemisp);  
    
                % save outputs
                outfname = ['sub_' prev_sub '_ses_' num2str(prev_ses) '_lssc_out.mat'];
...
if (RUN_DICE_SIMILARITY)
    ...
    save(fullfile(storepath, "lssc_avg_dice.mat"), 'avg_dice_subwise');
end
%%'
if (RUN_TEMPORAL_CORR)
    ...
    save(fullfile(storepath, "lssc_corr_means.mat"), 'mean_corr_within_subwise', 'mean_corr_across_subwise');
    % Set labels and title
    xlabel('Subjects');
    ylabel('Values');
