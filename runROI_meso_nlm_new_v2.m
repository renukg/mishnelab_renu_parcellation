function [L_all, mergedA_all] = runROI_meso_nlm_new_v2(cfg, PixxTime_dff, allregionspix, allregions, AllenFilePath, hemisp)
...
% Reorder line Allen
if ~isfield(cfg, 'ReorderLikeAllen')
...
for iclust=1:length(cfg.n_clust)
    ...
    for ithrcluster=1:length(cfg.thrcluster)
        ...
        if exist('stdA_all','var')
            ...
            if cfg.ReorderLikeAllen
                temp_labels = zeros(NROWS,NCOLS);
                for i= 1:size(mergedA,2)
                    temp_labels(mergedA(:,i)>0) = i;
                end
                temp_labels = ReorderLikeAllen(rot90(temp_labels, -1), AllenFilePath, hemisp);
                temp_labels = rot90(temp_labels, 1);
                clust_idx = unique(temp_labels);
                clust_idx(clust_idx==0) = [];
                mergedA = zeros(NROWS*NCOLS, length(clust_idx));
                for i1 = 1:length(clust_idx)
                    t_id = (temp_labels == clust_idx(i1));
            ...
            if cfg.ComputeTemporalCorr
                ...
                for cl = 1:num_clusters
                    ...
                    % for pixy = 1:length(cl_pix_ids)
                    %     corr_image = zeros(nR, nC);
                    %     corr_image(cl_pix_ids) = corr_within_mat(pixy, :);
                    % 
                    %     % Plot the correlation image
                    %     imagesc(corr_image);
                    %     colorbar;
                    %     title(sprintf('Correlations of Pixel %d in Cluster', cl_pix_ids(pixy)));
                    %     xlabel('X (pixels)');
                    %     ylabel('Y (pixels)');
                    %     caxis([-1, 1]); % Adjust the color limits for correlation values
                    %     axis image; % Keep the aspect ratio
                    % 
                    %     % Pause for a short time to create video effect
            ...
            save(fullfile(cfg.outputfilepath,[cfg.title_str,'_out.mat']), 'ROI', 'ROI_list','mergedA',...
                'NROWS','NCOLS','cfg','max_Psi','embedding_norm','t1','t2','t3','t4', ...
                'clusterwise_within_corr', 'clusterwise_across_corr');
            ...
            saveas(gcf, fullfile(cfg.outputfilepath,['ROIS_L_',cfg.title_str,'.png']));
