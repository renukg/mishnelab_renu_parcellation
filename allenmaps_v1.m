...
RUN_ALLEN_PROCESSING = 1;
RUN_ALLEN_REPORTING = 0;
        ...
        for i = 1:length(fileList) % processing each .nii file
            ...
            if ((i == 1 || strcmp(prev_sub, sub_value)) && i ~= length(fileList))    % runs belong to same subject
                ...
                for hemisp = 1:2
                    ...
                    % Create a figure
                    title_str = sprintf("Allen Parcel Response: Sub %s, Sess %d, Hem %d", prev_sub, prev_ses, hemisp);
                    figure;
                    hold on;
                    ...
                    title_str = sprintf("Allen Temp correlation: Sub %s, Sess %d, Hem %d", prev_sub, prev_ses, hemisp);
                    figure;
                    ...
                    % save outputs
                    outfname = ['sub_' prev_sub '_ses_' num2str(prev_ses) '_hemisp_' num2str(hemisp) '_allen_out.mat'];
