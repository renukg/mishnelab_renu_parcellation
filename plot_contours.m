...
if nargin < 3 || isempty(options) || isnumeric(options)
    if exist('options','var') && isnumeric(options)    % compatibility with previous version
        nrgthr = options;
        clear options
...
if ~isfield(options,'thr_method') || isempty(options.thr_method); options.thr_method = 'max'; end
if ~isfield(options,'nrgthr') || isempty(options.nrgthr); options.nrgthr = 0.995; end
if ~isfield(options,'maxthr') || isempty(options.maxthr); options.maxthr = 0.25; end
if ~isfield(options,'plot_bck_image') || isempty(options.plot_bck_image); options.plot_bck_image = true; end
